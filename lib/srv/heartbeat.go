...
func (h *Heartbeat) Run() error {
	...
	if h.ConnectedProxies != nil {
		proxyChange = utils.NewDebouncer(
			time.Second*5, time.Second*30,
		).Debounce(h.cancelCtx, h.ConnectedProxies.WaitForChange())
	}
...
// updateCurrent updates the current resource updating the nonce and nonceID
// if applicable.
